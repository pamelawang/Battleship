...
public class ComputerPlayer extends Player {
  ...
  private void setNextShot() { //resets aimAtX and aimAtY to next coordinate to be shot at
    ...
    if (direction == UP) {
      //x-coordinate is the same
        ...
        // then we assume that the boat has been sunk. Rest boatDirection to up.
  ...
  public int takeAShot (Player other) {
    ...
    } else if (lastShotResult == MISS) { //figuring out if direction is up/down/left/right
      direction++;
      setNextShot();
      try {
        shoot(other);
